...
public class WizardInfoController {
	...
	@ResponseBody
	public List<WizardInfo> getWizardsInfo(@RequestParam("supportType") final String supportType, final Locale locale) throws ClassNotFoundException {
		return getWizardsInfo(
				(Class<? extends ConfiguredBean>) Class.forName(supportType),
				locale);
	}

	public List<WizardInfo> getWizardsInfo(final Class<? extends ConfiguredBean> supportType, final Locale locale) {
		ArrayList<WizardInfo> infos = new ArrayList<WizardInfo>();
		ConfigBeanWizard<? extends ConfiguredBean>[] wizards = wizardProvider.getWizards(supportType);
		for (ConfigBeanWizard<? extends ConfiguredBean> wizard : wizards) {
			infos.add(new WizardInfo(
				wizard.getTemplate(),
				beanTypeResolver.resolveTypeName(wizard.getBeanType()),
				wizard.getWizardView(),
				msgSource.getMessage(wizard.getNameKey(), new Object[0], wizard.getBeanType().getSimpleName(), locale)));
		}
		return infos;
	}
}
