...
import com.catlogging.util.grok.GrokException;
...
public class GrokTest {
	...
	public void testSingleSimplePattern() throws GrokException {
		...
		assertEquals(true, r.getGroks().get("USERNAME").matcher("Tester")
				.matches());
		assertEquals(false, r.getGroks().get("USERNAME").matcher("Tester!")
				.matches());
	}
	...
	public void testSubPattern() throws GrokException {
		...
		assertEquals(true, r.getGroks().get("USER").matcher("Tester").matches());
		assertEquals(false, r.getGroks().get("USER").matcher("Tester!").matches());
	}
	...
	public void testSubPatternAttributes() throws GrokException {
		...
		assertEquals(true, g.matcher("Tester-").matches());

		// Test sub groks with attribute projection
		assertEquals(2, g.getGroupNames().get("a").intValue());
		GrokMatcher m = g.matcher("Tester-");
		assertEquals(true, m.matches());
		assertEquals("Tester", m.group(g.getGroupNames().get("a")));
		assertEquals("Tester", m.group("a"));
		assertEquals(false, g.matcher("Tester").matches());
	}
