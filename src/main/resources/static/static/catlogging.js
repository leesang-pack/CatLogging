...
(function() {
	...
	function getFieldsToRender(fieldsTypes, enabledViewerFields) {
		...
		for (var name in fieldsTypes) {
			if ($.catlogging.entriesIgnoreFields[name]) {
				continue;
			}
			fields.push(name);
		}
	...
	function printFieldValue(fieldType, fieldValue) {
		if (typeof fieldValue!="undefined") {
			...
			case "SEVERITY":
				...
				break;
			case "DATE":
				html = catlogging.ng.dateFilter(fieldValue, 'medium');
				break;
			case "OBJECT":
...
var catlogging = catlogging || $.catlogging;
catlogging.ng = {};
catlogging.config = {};
catlogging.DataTableBinder = function(rowsPath, cols) {
...
catlogging.objSize = function(obj) {
    var size = 0, key;
    for (key in obj) {
...
catlogging.moveUp = function(array, index) {
...
catlogging.moveDown = function(array, index) {
    var value = array[index];     
    var newPos = index + 1;
...
catlogging.DataSourceBuilder = function(savedData) {
	$.extend(this, {
		builders : [],
		joinMethod : "full",
		joinKeys : [ [ 0, 0 ] ]
	...
	for (var i = 0; i < this.builders.length; i++) {
		...
		this.builders[i] = new (catlogging.DataSourceBuilder[bData._type])(
				bData);
	}
	...
	this.build = function(params) {
		if (this.builders.length>0) {
			...
			return catlogging.DataSourceBuilder.joinDataTables(tables, this.joinMethod, this.joinKeys);
		} else {
...
catlogging.DataSourceBuilder.ElasticRequestBuilder = function(savedData) {
	$.extend(this, {
	...
	for (var i = 0; i < this.requestParts.length; i++) {
		...
		this.requestParts[i] = new (catlogging.DataSourceBuilder.ElasticRequestBuilder[data._type])(
				data);
	}
	this.build = function(params) {
...
catlogging.DataSourceBuilder.ElasticRequestBuilder.DateHistogramFacet = function(savedData) {
	$.extend(true, this, {
	...
	this.bindResponse = function(i, response) {
		...
		var binder = new catlogging.DataTableBinder(null,
				this.cols);
...
catlogging.DataSourceBuilder.ElasticRequestBuilder.StatsAgg = function(savedData) {
	$.extend(true, this, {
		_type : "StatsAgg",
		name : "",
		request : {
...
catlogging.DataSourceBuilder.ElasticRequestBuilder.TermsAgg = function(savedData) {
	$.extend(true, this, {
		_type : "TermsAgg",
		name : "",
		request : {
...
catlogging.DataSourceBuilder.joinDataTables = function(tables, joinMethod, joinKeys) {
...
catlogging.objToFormFields = function(obj, path) {
...
catlogging.ng.BeanWizardController = function($scope) {
...
catlogging.stripPrefix = function(source, prefix) {
	var target={};
	if (source) {
		for(var i in source) {
...
catlogging.containsKey = function(source, keys, startsWith) {
	if (!source) {
		return false;
	}
...
catlogging.hasKeysExpectOf = function(source, keys, startsWith) {
	if (!source) {
		return false;
	}
	if (!angular.isArray(keys)) {
...
catlogging.getFieldType = function (fields, fieldName) {
...
catlogging.has = function(obj, key) {
...
catlogging.get = function(obj, key) {
